...
if page == "📊 개요":
    ...
    if rows:
        ...
        # particle_size_primary_nm = TEM OR SEM 보유 논문 비율 (파생 필드 — JSONL에 없음)
        # 개별 TEM/SEM 필드를 대시보드에 노출하지 않고, 합산값만 표시
        if total_r > 0:
            n_primary = sum(
                1 for r in rows
                if (r.get("synthesis_conditions") or {}).get("particle_size_tem_nm") is not None
                or (r.get("synthesis_conditions") or {}).get("particle_size_sem_nm") is not None
            )
            fills["particle_size_primary_nm"] = n_primary / total_r * 100
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with ml_tabs[0]:
        ...
        _latest_histgbm = _latest_lgbm = _latest_dkl = _latest_cb = {}
        if os.path.exists(_ph_path):
            try:
                ...
                for _e in reversed(_ph_all):
                    ...
                    if not _latest_lgbm and _e.get("lgbm"):
                        _latest_lgbm = (_e["lgbm"].get("particle_size_primary_nm") or {})
        ...
        _mc2.metric(
            "LightGBM log-R²",
            f"{_latest_lgbm.get('log_r2', 'N/A'):+.3f}" if _latest_lgbm.get("log_r2") is not None else "N/A",
            f"MAE {_latest_lgbm.get('mae_nm', '?'):.1f} nm  |  n={_latest_lgbm.get('n', '?'):,}" if _latest_lgbm.get("mae_nm") else "",
            delta_color="off",
        )
        _mc3.metric(
            "CatBoost log-R² ★",
        ...
        if _ph:
            ...
            for _e in _ph:
                ...
                _lgbm = (_e.get("lgbm") or {}).get("particle_size_primary_nm") or {}
                ...
                _rows.append({
                    ...
                    "LightGBM log-R²": _lgbm.get("log_r2"),
                    "LightGBM MAE(nm)": _lgbm.get("mae_nm"),
            ...
            with _c1:
                ...
                _y_lgbm = _df_ph["LightGBM log-R²"].tolist()
                ...
                _fig1.add_trace(go.Scatter(
                    x=_xs, y=_y_lgbm, mode="lines+markers", name="LightGBM",
                    line=dict(color="#F5A623", width=2, dash="dashdot"),
            ...
            with _c2:
                ...
                _y_mae_l = _df_ph["LightGBM MAE(nm)"].tolist()
                ...
                _fig2.add_trace(go.Scatter(
                    x=_xs, y=_y_mae_l, mode="lines+markers", name="LightGBM MAE",
                    line=dict(color="#F5A623", width=2, dash="dashdot"),
                    marker=dict(size=8, symbol="triangle-up"),
                    connectgaps=True,
                ))
            ...
            with st.expander("📋 전체 이력 테이블"):
                _display_cols = ["세션", "샘플 수", "커버리지(%)",
                                 ...
                                 "LightGBM log-R²", "LightGBM MAE(nm)",
                                 "DKL-GP log-R²", "DKL-GP MAE(nm)",
                                 "CatBoost log-R²", "CatBoost MAE(nm)", "note"]
                _df_show = _df_ph[_display_cols].copy()
                for _col in ["HistGBM log-R²", "LightGBM log-R²", "DKL-GP log-R²", "CatBoost log-R²"]:
                    _df_show[_col] = _df_show[_col].apply(
                        lambda v: f"{v:+.3f}" if pd.notna(v) else "-"
                    )
                for _col in ["HistGBM MAE(nm)", "LightGBM MAE(nm)", "DKL-GP MAE(nm)", "CatBoost MAE(nm)"]:
                    _df_show[_col] = _df_show[_col].apply(
    ...
    with ml_tabs[1]:
        ...
        # HistGBM 피처 중요도: primary_nm · xrd_nm 2개만
        _IMP_SHOW = {"importance_particle_size_primary_nm.png", "importance_crystallite_size_xrd_nm.png"}
        importance_pngs = _sort_importance([
            p for p in glob.glob(os.path.join(MODEL_DIR, "importance_*.png"))
            if os.path.basename(p) in _IMP_SHOW
        ])
        # HistGBM SHAP: primary_nm · xrd_nm 2개만, 수치형 피처 한정
        _SHAP_SHOW = {"shap_particle_size_primary_nm.png", "shap_crystallite_size_xrd_nm.png"}
        shap_pngs = _sort_importance([
            p for p in glob.glob(os.path.join(MODEL_DIR, "shap_*.png"))
            if os.path.basename(p) in _SHAP_SHOW
        ])
        # CatBoost: primary_nm · xrd_nm 2개만
        _CB_SHOW = {"catboost_importance_particle_size_primary_nm.png", "catboost_importance_crystallite_size_xrd_nm.png",
                    "catboost_shap_particle_size_primary_nm.png",        "catboost_shap_crystallite_size_xrd_nm.png"}
        cb_importance_pngs = _sort_importance([
            p for p in glob.glob(os.path.join(MODEL_DIR, "catboost_importance_*.png"))
            if os.path.basename(p) in _CB_SHOW
        ])
        cb_shap_pngs = _sort_importance([
            p for p in glob.glob(os.path.join(MODEL_DIR, "catboost_shap_*.png"))
            if os.path.basename(p) in _CB_SHOW
        ])
        # DKL-GP: particle_size_primary_nm만
        dkl_result_pngs = [
            p for p in sorted(glob.glob(os.path.join(MODEL_DIR, "dkl_results_*.png")))
            if "particle_size_primary_nm" in os.path.basename(p)
        ]
        ...
        if not (importance_pngs or shap_pngs or cb_importance_pngs or cb_shap_pngs or dkl_result_pngs):
            ...
            cols_per_row = 2
            if importance_pngs:
                st.markdown("**HistGBM 피처 중요도** (particle_primary_nm · crystallite_xrd_nm)")
...
| 피처 | 의미 | 중요도(참고) |
|------|------|------|
| synthesis_method | 합성법 (sol-gel, hydrothermal 등) | 15.1% |
| capping_agent | 캡핑제 (입자 성장 억제) | 15.1% |
| solvent | 사용 용매 종류 | 10.2% |
| ce_concentration_M | Ce 전구체 농도 | 7.3% |
| ce_precursor | 세리아 전구체 종류 | 7.2% |
...
> **수치형 피처만 표시** (synthesis_method, ce_precursor, atmosphere 등 범주형 제외)
> 대상: **particle_size_primary_nm** (TEM+SEM) · **crystallite_size_xrd_nm** (XRD Scherrer)
...
- **점 색깔**: 빨강 = 해당 피처 값이 높음, 파랑 = 낮음
- **LightGBM SHAP와 차이**: CatBoost는 Ordered Boosting으로 target leakage를 방지 → 더 신뢰성 높은 피처 중요도

| 타깃 | log-R² | n |
|------|--------|---|
| particle_size_primary_nm | +0.092 | 2,800 |
| particle_size_tem_nm | +0.078 | 2,677 |
| crystallite_size_xrd_nm | +0.053 | 2,494 |
""")
                # 4열 그리드: [Importance primary | SHAP primary | Importance xrd | SHAP xrd]
                _cb_targets = ["particle_size_primary_nm", "crystallite_size_xrd_nm"]
                _cb_cols = st.columns(4)
                _col_idx = 0
                for tgt in _cb_targets:
                    imp = next((p for p in cb_importance_pngs if tgt in os.path.basename(p)), None)
                    shp = next((p for p in cb_shap_pngs       if tgt in os.path.basename(p)), None)
                    short = "primary_nm" if "primary" in tgt else "xrd_nm"
                    if imp and _col_idx < 4:
                        with _cb_cols[_col_idx]:
                            st.image(imp, caption=f"Importance — {short}", use_container_width=True)
                        _col_idx += 1
                    if shp and _col_idx < 4:
                        with _cb_cols[_col_idx]:
                            st.image(shp, caption=f"SHAP — {short}", use_container_width=True)
                        _col_idx += 1
...
""")
                for png in dkl_result_pngs:
                    cap = os.path.basename(png).replace("dkl_results_", "").replace(".png", "")
                    st.image(png, caption=f"DKL — {cap}", use_container_width=True)
...
""")
        ...
        if targeted_csvs:
            st.subheader("🎯 목표 크기별 합성 조건 (95% 예측 구간)")
            st.caption("12d_targeted_design.py 출력 | LightGBM Q2.5/Q97.5 분위 회귀 (95% 예측 구간)")
            with st.expander("📖 차트 해석 가이드", expanded=False):
                st.markdown("""
**이 차트는 목표 입자크기(10 / 30 / 60 nm)별로 가장 가능성 높은 합성 조건을 보여줍니다.**

#### 읽는 법
| 요소 | 의미 |
|------|------|
| **점 (●)** | Q50(중앙값) 예측 — 해당 조건에서 가장 기대되는 입자크기 |
| **가로 막대 (─)** | 95% 예측 구간 (Q2.5~Q97.5) — 실제값이 이 범위 안에 들어올 확률 약 95% |
| **빨간 점선** | 목표 입자크기 |
| **막대 폭이 좁을수록** | 예측 신뢰도가 높음 (모델이 확신하는 조건) |

#### Y축 레이블 읽기
```
hydrothermal          ← 합성 방법
(nh4)2ce(no3)6        ← Ce 전구체
T=164°C  pH=9  16h    ← 합성온도 / pH / 시간
```

#### 활용 방법
- **목표 크기에 점이 가장 가까운 조건**이 1순위 실험 후보
- **막대 폭이 좁고 목표에 가까운 조건**이 재현성 높은 최적 조건
- 여러 목표 크기 탭을 비교해 공통으로 등장하는 합성법·전구체 조합에 주목

> ⚠️ 이 예측은 기존 논문 데이터 기반입니다. 실제 실험 조건(도판트, 캡핑제 등)에 따라 결과가 달라질 수 있습니다.
""")
            if os.path.exists(targeted_png):
                st.image(targeted_png, caption="목표 크기별 95% 예측 구간", use_container_width=True)
            tabs_t = st.tabs([
...
""")
        ...
        # ── 섹션 1: 입자크기 능동학습 (DKL-GP σ 기반) ────────────────────────
        st.subheader("📏 입자크기 예측 개선을 위한 실험 제안")
        st.caption("σ(DKL-GP)가 가장 큰 조건 — 이 조건을 실험하면 입자크기 모델 개선 효과 최대")
        ...
        if al_dkl_files:
            try:
                ...
                cap = (f"{len(df_dkl)}개 제안 ({lbl}) | "
                       + ("predicted_mean_nm: 예측 중앙값, sigma_lower/upper_nm: 1σ 범위"
...
""")
        ...
        with tabs[3]:
            ...
            st.caption("Pearson 상관계수 (pairwise) · 데이터 소스: ceria_samples_merged.csv (샘플 단위)")
            HEAT_MAP = {
                ...
                "ph_synthesis":              "pH",
                "ce_concentration_M":        "Ce농도(M)",
                "synthesis_volume_mL":       "합성부피(mL)",
                "calcination_temperature_c": "하소온도(°C)",
                "calcination_time_h":        "하소시간(h)",
                "drying_temperature_c":      "건조온도(°C)",
            }
            _hm_src = pd.read_csv(MERGED_CSV, low_memory=False) if os.path.exists(MERGED_CSV) else df_a
            avail = {label: pd.to_numeric(_hm_src[col], errors="coerce")
                     for col, label in HEAT_MAP.items() if col in _hm_src.columns}
            if len(avail) < 3:
                st.info("수치 컬럼 부족")
            else:
                ...
                fig.update_layout(
                    ...
                    height=max(520, n_cols * 55), margin=dict(t=50, b=10),
                )
                st.plotly_chart(fig, use_container_width=True)
